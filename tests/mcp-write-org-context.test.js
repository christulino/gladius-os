...
describe('write_org_context — REST backing for MCP tool', () => {
  ...
  after(async () => {
    for (const id of created) {
      await api(`/organizations/${ORG_ID}/context/${id}`, { method: 'DELETE' })
    }
  })

  it('rejects create with missing required fields', async () => {
    const { status, data } = await api(`/organizations/${ORG_ID}/context`, {
      method: 'POST',
  ...
  it('creates an org context entry with all required fields', async () => {
    ...
    assert.equal(data.org_id, ORG_ID)
  ...
  it('filters org context by type', async () => {
    const { status, data } = await api(`/organizations/${ORG_ID}/context?types=standards`)
    assert.equal(status, 200)
  ...
  it('deletes org context entry', async () => {
    ...
    const { status, data } = await api(`/organizations/${ORG_ID}/context/${toDelete}`, {
      method: 'DELETE',
    })
    assert.equal(status, 200)
