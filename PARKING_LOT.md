...
**Status:** Resolved (Session 26)
**Context:** Added `.eslintrc.json` — ESM, Node globals, React/hooks plugin scoped to `admin-ui/src`, ignores for `dist/`, `node_modules/`, generated parser. `npx eslint .` runs clean.
...
**Status:** Partially resolved (Session 26)
**Context:** `work_item.comment_edited` and `work_item.comment_deleted` are now wired (PATCH/DELETE endpoints + HANDLED set + notification_defaults seed in migration 016). Remaining: `work_item.unlinked` (needs DELETE /links endpoint).
...
- One v1 event type still deferred: `work_item.unlinked` (needs DELETE /links endpoint). `work_item.comment_edited` and `work_item.comment_deleted` landed in Session 26.
