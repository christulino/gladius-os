      ...
      >
        <div className="flex items-center gap-1.5 min-w-0">
          {stage.is_entry_stage && (
            <span className="text-xs text-primary font-semibold flex-shrink-0">▶</span>
          )}
          ...
          {stage.is_terminal && !stage.counts_toward_throughput && (
            <span className="text-xs text-destructive ml-auto flex-shrink-0">✕</span>
          )}
          {stage.is_terminal && stage.counts_toward_throughput && (
            <span className="text-xs text-primary ml-auto flex-shrink-0">✓</span>
          )}
        </div>
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {stage.wip_limit && (
            <span className="text-xs text-muted-foreground">WIP {stage.wip_limit}</span>
          )}
          {stage.sla_hours && (
            <span className="text-xs text-muted-foreground">{stage.sla_hours}h SLA</span>
          )}
        </div>
      </div>
    </div>
...
function StageEditor({ stage, allStages, onSave, onDelete, onClose }) {
  ...
  if (!stage) return (
    <div className="w-[420px] flex-shrink-0 border-l border-border flex items-center justify-center">
